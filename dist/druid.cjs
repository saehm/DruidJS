  ...
  *[Symbol.iterator]() {
  ...
  set_row(row, values) {
    ...
    if (Matrix.isArray(values) && values.length === cols) {
      ...
      throw new Error(
        "Values not valid! Needs to be either an Array, a Float64Array, or a fitting Matrix!",
      );
    }
    return this;
  }
...
class HierarchicalClustering extends Clustering {
  ...
  constructor(points, parameters = {}) {
    ...
    if (
      ...
      this._matrix.shape[0] !== this._matrix.shape[1]
    ) {
...
class Cluster {
  ...
  constructor(id, left, right, dist, centroid, index, size, depth) {
    ...
    if (centroid !== undefined && centroid !== null) {
      ...
      if (!left || !right)
        throw new Error("If centroid is not given, left & right cannot be null!");
  ...
  *iterate() {
  ...
  toArray() {
    return this._container
      .sort((a, b) => (this._comparator(a.value, b.value) ? -1 : 1))
      .map((d) => d.element);
  }
  ...
  constructor(points, parameters = {}) {
    super(
      ...
      /** @type {ParametersKMeans} */ (
        Object.assign({ K: 4, metric: euclidean, seed: 1212 }, parameters)
      ),
    );
...
class KMedoids extends Clustering {
  ...
  constructor(points, parameters = {}) {
    super(
      points,
      Object.assign({ K: 4, max_iter: null, metric: euclidean, seed: 1212 }, parameters),
    );
  ...
  constructor(points, parameters = {}) {
    super(
      ...
      /** @type {ParametersMeanShift} */ (
        Object.assign(
          { seed: 1212, metric: euclidean, bandwidth: 0, kernel: "gaussian" },
          parameters,
        )
      ),
    );
...
class XMeans extends Clustering {
  ...
  _run() {
    ...
    while (K < this._parameters.K_max) {
      ...
      /** @type {SplitResult[]} */
      const accepted_splits = split_results.filter(
        (result) => result.bic_children > result.bic_parent,
      );
      ...
      accepted_splits.sort(
        (a, b) => b.bic_children - b.bic_parent - (a.bic_children - a.bic_parent),
      );
  ...
  transform() {
    ...
    for (let _col = 0; _col < d; ++_col) {
      ...
      if (d_ab !== 0) {
        ...
        dist = (a, b) =>
          Math.sqrt(old_dist(a, b) ** 2 - (Y.entry(a, _col) - Y.entry(b, _col)) ** 2);
      }
    }
    // return embedding.
    this.Y = Y;
...
class Annoy extends KNN {
  ...
  search(query, k = 5) {
    ...
    const result = [];
    while (best.length > 0) {
      const item = /** @type {{ element: { index: number; distance: number }; value: number }} */ (
        best.pop()
      );
      result.push({
...
class HNSW extends KNN {
  ...
  constructor(
    ...
    /** @type {Function} */
    this._select = this._parameters.heuristic
      ? this._select_heuristic.bind(this)
      : this._select_simple.bind(this);
...
class KDTree extends KNN {
  ...
  search(t, k = 5) {
    ...
    while (best.length > 0) {
      const item =
        /** @type {{ element: { point: ElementWithIndex<T>; distance: number }; value: number }} */ (
          best.pop()
        );
      result.push({
  ...
  constructor(elements, parameters = {}) {
    ...
    const N =
      this._elements instanceof Matrix
        ? /** @type {any} */ (this._elements).shape[0]
...
class ISOMAP extends DR {
  ...
  constructor(X, parameters = {}) {
    ...
    if (this._parameters.neighbors === -Infinity) {
      this.parameter(
        "neighbors",
        Math.min(Math.max(Math.floor(this.X.shape[0] / 10), 2), this._N - 1),
      );
    }
...
class SQDMDS extends DR {
  ...
  *generator(iterations = 500) {
   ...
   */
  _step(i, iterations) {
    if (this._LR_init === undefined || this._offset === undefined)
  ...
  _nestrov_iteration(distance_exaggeration) {
    if (!this._momentums || !this._grads || this._LR === undefined)
      throw new Error("Call init() first!");
  ...
  _fill_MDS_grads(Y, grads, exaggeration = false, zero_grad = true) {
    if (!this._HD_metric || !this._HD_metric_exaggeration || !this._add)
      throw new Error("Call init() first!");
    if (zero_grad) {
  ...
  _ABCD_grads(a, b, c, d, d_ab, d_ac, d_ad, d_bc, d_bd, d_cd, p_ab, sum_LD_dist) {
    if (!this._minus || !this._add || !this._mult || !this._sub_div)
      throw new Error("Call init() first!");
    ...
    const gA = mult(
      minus(
        mult(add(sub_div(a, b, d_ab), sub_div(a, c, d_ac), sub_div(a, d, d_ad)), ratio),
        sub_div(a, b, d_ab),
      ),
      twice_ratio,
    );
    const gB = mult(
      minus(
        mult(add(sub_div(b, a, d_ab), sub_div(b, c, d_bc), sub_div(b, d, d_bd)), ratio),
        sub_div(b, a, d_ab),
      ),
      twice_ratio,
    );
    const gC = mult(
      add(sub_div(c, a, d_ac), sub_div(c, b, d_bc), sub_div(c, d, d_cd)),
      ratio * twice_ratio,
    );
    const gD = mult(
      add(sub_div(d, a, d_ad), sub_div(d, b, d_bd), sub_div(d, c, d_cd)),
      ratio * twice_ratio,
    );
    return [gA, gB, gC, gD];
  }
   ...
   */
  __sub_div(d) {
    return /** @type {(x: Float64Array, y: Float64Array, div: number) => Float64Array} */ (
      x,
      y,
      div,
    ) => {
...
class TriMap extends DR {
  ...
  init(pca = null, knn = null) {
    ...
    const { triplets, weights } = this._generate_triplets(
      this.n_inliers,
      this.n_outliers,
      this.n_random,
    );
  ...
  _sample_knn_triplets(P, nbrs, n_inliers, n_outliers) {
    ...
    for (let i = 0; i < N; ++i) {
      ...
      for (let j = 0; j < n_inliers; ++j) {
        ...
        const rejects = [
          i,
          ...Array.from(sort_indices.slice(0, j + 2)).map((idx) => nbrs.entry(i, idx)),
        ];
...
class UMAP extends DR {
      ...
      if (non_zero_dist_length >= local_connectivity) {
        if (index > 0) {
          ...
          if (interpolation > SMOOTH_K_TOLERANCE) {
            rho +=
              interpolation * (non_zero_dist[index].distance - non_zero_dist[index - 1].distance);
          }
        } else {
  ...
  init() {
    ...
    this._epochs_per_negative_sample = this._epochs_per_sample.map(
      (d) => d * _negative_sample_rate,
    );
  ...
  _optimize_layout(head_embedding, tail_embedding, head, tail) {
    ...
    for (let i = 0, n = epochs_per_sample.length; i < n; ++i) {
      if (epoch_of_next_sample[i] <= this._iter) {
        ...
        const n_neg_samples =
          (this._iter - epoch_of_next_negative_sample[i]) / epochs_per_negative_sample[i];
        for (let p = 0; p < n_neg_samples; ++p) {
          ...
          if (dist > 0) {
            const grad_coeff =
              (2 * _repulsion_strength * b) / ((0.01 + dist) * (a * dist ** b + 1));
            for (let d = 0; d < dim; ++d) {
              const grad_d = clip(grad_coeff * (current[d] - other[d])) * alpha;
              current[d] += grad_d;
              other[d] -= grad_d;
...
exports.bray_curtis = bray_curtis;
...
exports.goodman_kruskal = goodman_kruskal;
...
exports.wasserstein = wasserstein;
exports.yule = yule;
