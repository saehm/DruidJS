...
// scale parameter
const c=new Float64Array(o);for(let t=0;t<o;++t)c[t]=Math.max((_.entry(t,3)+_.entry(t,4)+_.entry(t,5)+_.entry(t,6))/4,1e-10);const u=this._find_p(_,c,l);let d=this._sample_knn_triplets(u,l,t,e),p=d.shape[0];const f=new Float64Array(p);for(let t=0;t<p;++t){const e=d.entry(t,0),r=d.entry(t,2);f[t]=s(n.row(e),n.row(r))}let m=this._find_weights(d,u,l,f,c);if(r>0){const{random_triplets:t,random_weights:e}=this._sample_random_triplets(n,r,c);d=d.concat(t,"vertical"),m=Float64Array.from([...m,...e])}p=d.shape[0];let y=-1/0;for(let t=0;t<p;++t)isNaN(m[t])&&(m[t]=0),y<m[t]&&(y=m[t]);let w=-1/0;for(let t=0;t<p;++t)m[t]/=y,m[t]+=1e-4,m[t]=Math.log(1+i*m[t]),w<m[t]&&(w=m[t]);for(let t=0;t<p;++t)m[t]/=w;return{triplets:d,weights:m}}
/**
...
/**
     ...
     * @param {Float64Array} sig - Scaling factor for the distances
     */_sample_random_triplets(t,e,r){const s=this._metric,i=this._randomizer,n=t.shape[0],o=new Matrix(n*e,3),a=new Float64Array(n*e);for(let h=0;h<n;++h){const l=h*e,_=[...linspace(0,h-1),...linspace(h+1,n-1)];for(let n=0;n<e;++n){let[e,c]=i.choice(_,2),u=Math.exp(-(s(t.row(h),t.row(e))**2)/(r[h]*r[e]));u<1e-20&&(u=1e-20);let d=Math.exp(-(s(t.row(h),t.row(c))**2)/(r[h]*r[c]));d<1e-20&&(d=1e-20),u<d&&([e,c]=[c,e],[u,d]=[d,u]);const p=l+n;o.set_entry(p,0,h),o.set_entry(p,1,e),o.set_entry(p,2,c),a[p]=u/d}}return{random_triplets:o,random_weights:a}}
/**
     * Computes the gradient for updating the embedding.
     * @param {Matrix} Y - The embedding
     */_grad(t){const e=this.n_inliers,r=this.n_outliers,s=this.triplets,i=this.weights,[n,o]=t.shape,a=s.shape[0],h=new Matrix(n,o,0);let l=new Array(o).fill(0),_=new Array(o).fill(0),c=1,u=1,d=0,p=0;const f=n*e*r;for(let e=0;e<a;++e){const[n,a,m]=s.row(e);
// update y_ij, y_ik, d_ij, d_ik
if(e%r==0||e>=f){c=1,u=1;for(let e=0;e<o;++e){const r=t.entry(n,e),s=t.entry(a,e),i=t.entry(m,e);l[e]=r-s,_[e]=r-i,c+=l[e]**2,u+=_[e]**2}
// update y_ik and d_ik only
}else{u=1;for(let e=0;e<o;++e){const r=t.entry(n,e),s=t.entry(m,e);_[e]=r-s,u+=_[e]**2}}c>u&&++d,p+=i[e]/(1+u/c);const y=(i[e]/(c+u))**2;for(let t=0;t<o;++t){const e=l[t]*u*y,r=_[t]*c*y;h.set_entry(n,t,h.entry(n,t)+e-r),h.set_entry(a,t,h.entry(a,t)-e),h.set_entry(m,t,h.entry(m,t)+r)}}return{grad:h,loss:p,n_viol:d}}
/**
     * 
...
/**
     * computes the tree.
     */do(){const t=this._n,e=this._d_min,r=this._distance_matrix,s=this._clusters,i=this._c_size,n=this._linkage;let o=null;for(let a=0,h=t-1;a<h;++a){let a=0;for(let s=0;s<t;++s)r.entry(s,e[s])<r.entry(a,e[a])&&(a=s);let h=e[a],l=s[a][0],_=s[h][0],c=l.isLeaf?[l.index]:l.index,u=_.isLeaf?[_.index]:_.index,d=c.concat(u),p=new Cluster(this._id++,l,_,r.entry(a,h),null,d);l.parent=p,_.parent=p,s[a].unshift(p),i[a]+=i[h];for(let e=0;e<t;++e)switch(n){case"single":r.entry(a,e)>r.entry(h,e)&&(r.set_entry(e,a,r.entry(h,e)),r.set_entry(a,e,r.entry(h,e)));break;case"complete":r.entry(a,e)<r.entry(h,e)&&(r.set_entry(e,a,r.entry(h,e)),r.set_entry(a,e,r.entry(h,e)));break;case"average":const t=(i[a]*r.entry(a,e)+i[h]*r.entry(h,e))/(i[a]+i[e]);r.set_entry(e,a,t),r.set_entry(a,e,t)}r.set_entry(a,a,1/0);for(let e=0;e<t;++e)r.set_entry(e,h,1/0),r.set_entry(h,e,1/0);for(let s=0;s<t;++s)e[s]===h&&(e[s]=a),r.entry(a,s)<r.entry(a,e[a])&&(e[a]=s);o=p}return o}}class Cluster{constructor(t,e,r,s,i,n,o,a){return this.id=t,this.left=e,this.right=r,this.dist=s,this.index=n,this.size=o??e.size+r.size,this.depth=a??1+Math.max(e.depth,r.depth),this.centroid=i??this._calculate_centroid(e,r),this.parent=null,this}_calculate_centroid(t,e){const r=t.size,s=e.size,i=t.centroid,n=e.centroid,o=this.size,a=t.centroid.length,h=new Float64Array(a);for(let t=0;t<a;++t)h[t]=(r*i[t]+s*n[t])/o;return h}get isLeaf(){return 0===this.depth}leaves(){if(this.isLeaf)return[this];const t=this.left,e=this.right;return(t.isLeaf?[t]:t.leaves()).concat(e.isLeaf?[e]:e.leaves())}descendants(){if(this.isLeaf)return[this];const t=this.left.descendants(),e=this.right.descendants();return t.concat(e).concat([this])}}
/**
 * @class
 * @alias KMeans
 */class KMeans{
...
/**
     ...
     * @returns {LSP} 
     */init(t=MDS,e=[],r=BallTree){if(this._is_initialized)return this;const s=this.X,i=this._N,n=this.parameter("k"),o=this._d,a=this._metric,h=this.parameter("control_points"),l=new KMedoids(s,h,null,a).get_clusters().medoids,_=new Matrix(h,i,"zeros");l.forEach(((t,e)=>{_.set_entry(e,t,1)}));const c=new t(Matrix.from(l.map((t=>s.row(t)))),...e,o).transform(),u=s.to2dArray,d=new r(u,a),p=new Matrix(i,i,"I"),f=-1/n;u.forEach(((t,e)=>{for(const{index:r}of d.search(t,n).iterate())e!==r&&p.set_entry(e,r,f)}));const m=p.concat(_,"vertical"),y=new Matrix(i,o,"zeros").concat(c,"vertical");return this._A=m,this._b=y,this._is_initialized=!0,this}
/**
     * Computes the projection.
     * @returns {Matrix} Returns the projection.
...
/**
     ...
     * @see {@link https://arxiv.org/pdf/2009.01512.pdf}
     */
constructor(t,e=5,r=2,s=euclidean,i=1212){return super(t,r,s,i),super.parameter_list=["max_halves"],this.parameter("max_halves",e),[this._N,this._D]=this.X.shape,this}
/**
     * initializes SAMMON. Sets all projcted points to zero, and computes a minimum spanning tree.
     */init(t="random",e=null){const r=this._N,s=this._d;if("random"===t){const t=this._randomizer;console.log(t),this.Y=new Matrix(r,s,(()=>t.random))}else this.Y=t.transform(this.X);const i=this.Y;e||(e=new Matrix(r,r));const n=this._metric;let o=new Matrix(r,r),a=new Matrix(r,r);for(let t=0;t<r;++t){const s=i.row(t);for(let h=t;h<r;++h){let r=t===h?1:n(s,i.row(h)),l=1/r;if(o.set_entry(t,h,r),o.set_entry(h,t,r),a.set_entry(t,h,l),a.set_entry(h,t,l),!e){let r=t===h?1:n(X.row(t),X.row(h));e.set_entry(t,h,r),e.set_entry(h,t,r)}}}let h=e._apply(1,((t,e)=>e/t)),l=e.sub(o),_=l._apply(2,((t,e)=>Math.pow(t,e))).mult(h);return console.log(_),this._distance_matrix=o,this._distance_inverse_matrix=a,this._Distance_matrix=e,this._Distance_inverse_matrix=a,this._delta=l,this._ones=new Matrix(r,s,1),this._E=_.sum,this}
/**
     * @private
     * @param {Matrix} A
     * @returns {Matrix} 
     */__distance_matrix(t){const e=this._metric,r=t.shape[0],s=new Matrix(r,r);for(let i=0;i<r;++i){const n=t.row(i);for(let o=i;o<r;++o){let r=i===o?1:e(n,t.row(o));s.set_entry(i,o,r),s.set_entry(o,i,r)}}return s}
/**
     * Transforms the inputdata {@link X} to dimenionality 2.
     */transform(t=20){this._is_initialized||this.init();for(let e=0;e<t;++e)console.log([...this.Y]),this._step();return this.projection}*generator(){this._is_initialized||this.init();for(let t=0;t<max_iter;++t)this._step(),yield this.projection;return this.projection}_step(){const t=this.parameter("max_halves");let e=this._distance_matrix,r=this._distance_inverse_matrix,s=this._Distance_matrix,i=this._Distance_inverse_matrix,n=this._ones,o=this._E,a=this.Y,h=r.sub(i),l=h.dot(n),_=h.dot(a).sub(a.mult(l)),c=r._apply(3,((t,e)=>Math.pow(t,e))),u=a._apply(2,((t,e)=>Math.pow(t,e))),d=c.dot(u).sub(l).sub(a.mult(2).mult(c.dot(a))).add(u.mult(c.dot(n)));d=d._apply(null,(t=>Math.abs(t)));let p=_.divide(d),f=a.clone();for(let n=0;n<t;++n){if(a=f.add(p),e=this.__distance_matrix(a),r=e._apply(1,((t,e)=>e/t)),h=s.sub(e),h._apply(2,((t,e)=>Math.pow(t,e))).mult(i).sum<o)break;p=p.mult(.5)}this.Y=a}}var t="0.3.12";export{BallTree,DisjointSet,FASTMAP,Heap,Hierarchical_Clustering,ISOMAP,KMeans,KMedoids,LDA,LLE,LSP,LTSA,MDS,Matrix,OPTICS,PCA,Randomizer,SAMMON,TSNE,TopoMap,TriMap,UMAP,canberra,chebyshev,cosine,distance_matrix,euclidean,euclidean_squared,k_nearest_neighbors,kahan_sum,linspace,manhattan,max,neumair_sum,norm,powell,qr,simultaneous_poweriteration$1 as simultaneous_poweriteration,t as version};
